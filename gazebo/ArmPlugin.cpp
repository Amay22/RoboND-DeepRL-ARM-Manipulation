...
bool ArmPlugin::updateAgent()
{
    ...
    // coefficient of actionVelDelta and actionJointDelta value
	...
	/
	*/
	float velocity = vel[action/2] + c * actionVelDelta; // Set joint velocity based on whether action is even or odd.
	...
	/
	*/
	float joint = ref[action/2] + c * actionJointDelta; // Set joint position based on whether action is even or odd.
		...
		*/
		...
		if(!checkGroundContact)
		{
			...
			if( episodeFrames > 1 )
			{
				...
				avgGoalDelta  = (avgGoalDelta * 0.4f) + (distDelta * 0.6f);
